...
jqSimpleConnect.connect = function(elementA, elementB, options) {
    ...
    if(elementA == null || jQuery(elementA).width() == 0 ||
       elementB == null || jQuery(elementB).width() == 0) {
       return null;
       }

    elementA = jQuery(elementA);
    if(elementA.width() > 1) elementA = elementA.first();
    elementB = jQuery(elementB);
    if(elementB.width() > 1) elementB = elementB.first();
